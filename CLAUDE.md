...
## WebGL home-page rebuild: tried twice, discarded twice — don't just redo it

The user has twice asked to rebuild the home page's floating photo collage (33 DOM tiles + CSS-translate parallax, driven by `onHomeMouseMove`) to look more like the michaelgatt.com reference (a real Three.js WebGL 3D scene with camera perspective, particles, GPU depth-of-field blur):
...
2. **Attempt 2**: full WebGL rebuild via Three.js (`EffectComposer`/`RenderPass`/`BokehPass` from `unpkg.com/three@0.160.0`, canvas replacing the old tile loop, camera-driven parallax instead of per-tile translate, real particles). Pushed to branch `webgl-home-experiment`, got a working Vercel preview, but after actually looking at it the user said it felt nothing like the reference and didn't want it — **branch deleted** (`git push origin --delete webgl-home-experiment`), main untouched. Two real implementation bugs were found/fixed along the way if this gets attempted a third time: (a) this template engine doesn't reliably apply a literal `style="width:100%"` string on a plain element — size the canvas from JS instead; (b) remove `onMouseMove`/`onHomeMouseMove` from the `<main>` home element before adding independent WebGL mouse tracking, or every mouse move re-renders the whole component tree and tears down the canvas.

Takeaway: don't assume the user wants either the CSS-3D or WebGL direction rebuilt again just because they mentioned michaelgatt.com once — the gap turned out to be more about overall mood/art-direction than the parallax technique, and both prior attempts missed it. If asked again, get much more specific about what "feels different" before writing code — ask for a second reference site, or which specific quality (composition density, color grading, motion speed, camera angle) is off, rather than re-implementing the same camera-parallax approach a third time.
