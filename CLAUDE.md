...
## WebGL home-page rebuild: tried three times, discarded three times — stop redoing it, ask instead

The user has repeatedly asked to rebuild the home page's floating photo collage (33 DOM tiles + CSS-translate parallax, driven by `onHomeMouseMove`) to look more like the michaelgatt.com reference (a real Three.js WebGL 3D scene with camera perspective, particles, GPU depth-of-field blur). **Do not attempt a fourth rebuild without first getting much more specific feedback than "make it like the reference" — see Takeaway below.**

1. **Attempt 1** (prior session): lighter CSS-3D-only version (`perspective` + `translateZ` for depth-scale/blur, slight mouse-driven `rotateX/Y`, no real WebGL). Built, tested in an untracked local branch, looked reasonable, but discarded entirely at the user's request — branch deleted, nothing committed, no trace in this repo.

2. **Attempt 2**: full WebGL rebuild via Three.js (`EffectComposer`/`RenderPass`/`BokehPass` from `unpkg.com/three@0.160.0`, canvas replacing the old tile loop, camera-driven parallax, real particles). Pushed to branch `webgl-home-experiment`. User's specific complaint after testing: photos "all clustered in one place," visible flicker/glitching, and barely any response to mouse movement — nothing like the reference's dramatic "swooshing" motion. Root causes diagnosed: (a) this template engine doesn't reliably apply a literal `style="width:100%"` string on a plain element — size the canvas from JS instead; (b) the home page's `setInterval` (rotates a category label every 2.6s, in `componentDidMount`) triggers a full re-render, and this engine appears to recreate DOM nodes on re-render rather than reconciling — since the canvas lived *inside* the re-rendered tree, it was being torn down and rebuilt every 2.6s, resetting the camera and reloading all 33 textures each time. That's the flicker AND the "doesn't respond to mouse" complaint in one bug. **Branch deleted.**

3. **Attempt 3** (`webgl-home-v2`): fixed both root causes from attempt 2 — canvas now created once and appended directly to `document.body`, entirely outside the component's render tree (position/size synced every frame to the still-churned-but-now-just-a-cheap-empty-anchor `.pf-float-wrap` div via `getBoundingClientRect()`, so re-renders no longer affect it at all); mouse-driven camera movement multipliers increased roughly 4x plus added camera roll for a more dramatic swing; wider 3D spread (worldW/H roughly 2x) so photos read as less clustered. Pushed, got a working preview, but the user said it still wasn't it and asked to stop. **Branch deleted, not merged.**

Takeaway: the gap is not the parallax technique or a fixable bug — three structurally different implementations (CSS-3D, WebGL v1, WebGL v2 with real bugs fixed) all missed what the user wants, so the mismatch is almost certainly about overall mood/art-direction (composition density, color grading, camera framing, motion character) rather than something more code can address by iterating on the same approach. If asked again, don't jump straight to implementation — ask what specifically is off, or for a different/clearer reference, before writing any code.
